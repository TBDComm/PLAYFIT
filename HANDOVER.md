...
| 2026-03-28 | fix(guidelines): 6 violations from Gemini code review — FeedbackButtons supabase to module scope, tag-weights factory→serviceSupabase, LibraryPickerModal aria-label+role=dialog on inner modal, SettingsClient weight btn aria-label, result card box-shadow transition | `FeedbackButtons.tsx`, `api/tag-weights/route.ts`, `LibraryPickerModal.tsx`, `SettingsClient.tsx`, `result/[id]/page.module.css` |
...
| 2026-03-28 | ux(home): 5 UX improvements — auth-loading submit disabled, steam-linked subtitle, client-side URL validation, manual notice rewrite, sample card moved before form | `app/components/RecommendationForm.tsx`, `app/page.tsx` |
| 2026-03-28 | ux(home): move sample card below form (Hero → Form → Sample) | `app/page.tsx` |
| 2026-03-28 | fix(saved-games): wrap in section with proper padding/bg — was bare div with inline marginTop | `SavedGames.tsx`, `page.module.css` |
| 2026-03-28 | fix(design): 7-point completeness audit — CSS variables, skeleton animation, border-radius, backdrop | `globals.css`, `LoadingOverlay.module.css`, `LibraryPickerModal.module.css`, `page.module.css`, `SavedGames.tsx`, `Header.module.css` |
